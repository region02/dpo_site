...
<section id="feedback" class="feedback">
    <div class="feedback__wrapper px-3 lg:px-12 pt-28">
        <div class="feedback__container bg-white bg-opacity-10 rounded-[15px] px-2 sm:px-10 py-14 min-h-[424px]">
            ...
            <script>
                document.addEventListener('alpine:init', () => {
                 ...
                 async function handleSubmit(e) {
                    e.preventDefault();
                    Alpine.store('modalOpen').toggle();
                    Alpine.store('modalOpen').setStatus("pending");
                    const formData = new FormData(e.target);
                    const res = await fetch("{{route('index.questions')}}", {
                        method: "POST",
                        headers: {
                    ...
                    data = await res.json();
                    if (data.status === 'feedback-ok') {
                        Alpine.store('modalOpen').setStatus("success");
                        e.target.reset();
                    } else {
                        Alpine.store('modalOpen').setStatus("failed");
                    }
            ...
            <form
                class="feedback__form  w-[100%] max-w-[100%] 2xl:max-w-[80%] flex flex-col justify-center gap-[30px] 3xl:gap-[40px]"
                ...
                <div class=" radio-party flex  justify-center flex-col gap-[20px] sm:gap-[15px] sm:flex-row">
                    <div class="flex items-center me-4">
                        <input id="messanger-bt" type="radio" value="мессенджер" name="feedback_type"
                            class=" relative w-[25px] h-[25px]      bg-[#255C96] border-[#fff]
                        ...
                        <label for="messanger-bt"
                            class="ms-2 text-sm 3xl:text-[20px] font-medium text-[#fff] ">мессенджер</label>
                    </div>
                    <div class="flex items-center me-4">
                        <input id="email-bt" type="radio" value="почта" name="feedback_type"
                            class=" relative w-[25px] h-[25px]      bg-[#255C96] border-[#fff]
                                                                                            border-[2px] z-0 checked:border-transparent checked:bg-transparent
                                                                                            before:scale-0
                                                                                            checked:before:scale-100
                                                                                            ...
                                                                                            after:z-11 after:left-[-2px] after:top-[-2px]">
                        <label for="email-bt"
                            class="ms-2 text-sm 3xl:text-[20px] font-medium text-[#fff] ">почта</label>
                    </div>
                    <div class="flex items-center me-4">
                        <input id="phone-bt" type="radio" checked value="телефон" name="feedback_type"
                            class=" relative w-[25px] h-[25px]      bg-[#255C96] border-[#fff]
                                                                                            border-[2px] z-0 checked:border-transparent checked:bg-transparent
                                                                                            before:scale-0
                                                                                            checked:before:scale-100
                                                                                            ...
                                                                                            after:z-11 after:left-[-2px] after:top-[-2px]">
                        <label for="phone-bt"
                            class="ms-2 text-sm 3xl:text-[20px] font-medium text-[#fff] ">телефон</label>
                    </div>
                ...
                <div class=" relative  ">
                    ...
                    <textarea x-data="{
                                ...
                                }" x-init="resize()" @input="resize()" type="text" id="question" name="question"
                        autocomplete="off"
                ...
                <div>
                    <div class="inline-flex items-center gap-3">
                        <label class="relative flex items-center  rounded-full cursor-pointer" for="agreement_1">
                ...
                <div>
                    <div class="inline-flex items-center gap-3">
                        <label class="relative flex items-center  rounded-full cursor-pointer" for="agreement_2">
    ...
    <div x-data="{ modalOpen: $store.modalOpen.on, status: $store.modalOpen.status }"
         x-init="$watch('$store.modalOpen.on', value => modalOpen = value); $watch('$store.modalOpen.status', value => status = value);"
        ...
        <template x-teleport="body">
            <div x-show="modalOpen" class="fixed top-0 left-0 z-[99] flex items-center justify-center w-screen h-screen"
                ...
                <div x-show="modalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                    ...
                    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click="$store.modalOpen.toggle()"
                ...
                <div x-show="modalOpen" x-trap.inert.noscroll="modalOpen" x-transition:enter="ease-out duration-300"
                    ...
                    <div class="flex items-center justify-between gap-[30px]  pb-3">
                        ...
                        <div class="gradient-border before:p-[5px] relative w-12 h-12">
                            <button @click="$store.modalOpen.toggle()"
                    ...
                    <template x-if="status === 'success'">
                        <div class="flex flex-col items-center">
                            <lord-icon class="max-w-[250px] max-h-[250px] w-[160px] h-[160px] sm:w-[250px] sm:h-[250px]"
                                src="https://cdn.lordicon.com/dangivhk.json" trigger="loop" stroke="bold"
                                colors="primary:#121331,secondary:#da22ff">
                            </lord-icon>
                            <h4 class="card__subtitle text-[#fff] text-[20px] text-center font-[400]">Ваша заявка принята,
                                скоро мы с вами свяжемся</h4>
                            <h4 class="card__subtitle text-[#fff] text-[20px] text-center font-[400]">Скоро мы с вами
                                свяжемся</h4>
                        </div>
                    </template>
                    <template x-if="status === 'pending'">
                        <div class="flex flex-col items-center">
                            ...
                            colors="primary:#da21ff,secondary:#9633ee"
